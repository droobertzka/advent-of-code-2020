...
isValidPartOne (min, max, char, pw) =
  let conditions = [hasMinChars min char pw, hasMaxChars max char pw]
  in  null $ filter (== False) conditions

partOne ps = length $ filter (== True) $ map isValidPartOne ps
...
isValidPartTwo (i, j, char, pw) =
  ...
  in  if isAtFirstIndex then not isAtSecondIndex else isAtSecondIndex

partTwo ps = length $ filter (== True) $ map isValidPartTwo ps
