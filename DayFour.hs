...
validateYear min max year =
  let yearAsInt = read year :: Int in yearAsInt >= min && yearAsInt <= max
...
validateHairColor (h : rest) =
  h == '#' && length rest == 6 && all (`elem` "abcdef01234567890") rest

validateEyeColor = (`elem` ["amb", "blu", "brn", "gry", "grn", "hzl", "oth"])

validatePassportId str = length str == 9 && all (`elem` "0123456789") str
...
parsePassport acc curr =
  if null curr then acc ++ [curr] else (init acc) ++ [(last acc) ++ " " ++ curr]

validateField reqFields field =
  filter (/= (takeWhile (/= ':') field)) reqFields

hasRequiredFields str =
  (== 0) . length $ foldl validateField requiredFields (words str)

parseInputs :: [String] -> [String]
parseInputs = foldl parsePassport [""]

partOne xs = length $ filter hasRequiredFields $ parseInputs xs


-- Part 2

validateFieldValue field =
  let [keyAsText, valueAsText] = T.splitOn (T.pack ":") (T.pack field)
...
partTwo xs =
  let passportsWithReqFields = filter hasRequiredFields $ parseInputs xs
  in  length $ filter (all validateFieldValue . words) passportsWithReqFields
