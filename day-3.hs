...
getUseableIndex n str =
  let strLen = length str in n - (n `div` strLen * strLen)
...
calcNumHashes (x, y) numHashes strings (slopeX, slopeY) =
  if y >= length strings
    then numHashes
      ...
      let nextNumHashes =
            if isHash x (strings !! y) then numHashes + 1 else numHashes
      in  calcNumHashes (x + slopeX, y + slopeY)
                        nextNumHashes
                        strings
                        (slopeX, slopeY)
...
partOne strings = calcNumHashes start 0 strings partOneSlope
...
partTwoSlopes = [(1, 1), (3, 1), (5, 1), (7, 1), (1, 2)]
